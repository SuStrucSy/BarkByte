...
const getStatusCode = (error?: Error | null): number => {
  if (!error) return 500;
  if (
    "response" in error &&
    error.response &&
    "status" in (error.response as any)
  ) {
    return (error.response as any).status ?? 500;
  }
  return 500;
};
...
const getErrorMessage = (error?: Error | null): string => {
  ...
  if ("response" in error && error.response) {
    ...
    const detail = (error.response as any).data?.detail;
    switch (status) {
      case 400:
