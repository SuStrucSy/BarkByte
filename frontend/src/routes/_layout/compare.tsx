...
function formatInlineCompareValue(
  ...
  if (key === "connector" && typeof value === "boolean") {
    return { text: value ? "has connector" : "has no connector", isMissing: false };
  }
...
function getCompareFields(specimen?: SpecimenPublic): CompareField[] {
  ...
  return uniqueKeys
    .map((key) => ({
      ...
      render: (currentSpecimen) => formatInlineCompareValue(key, currentSpecimen[key]),
    }));
}
...
function ComparePage() {
  ...
  const clearSlot = (slotIndex: number) => {
    setSelectedIds((current) =>
      current.map((selectedId, index) => (index === slotIndex ? null : selectedId)),
    );
  };
