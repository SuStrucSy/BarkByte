...
export function getDisplayText(value: unknown, fallback = "Unnamed"): string {
	if (value === null || value === undefined || value === "") {
		return "—";
	}
	...
	if (typeof value === "string") {
		...
		return isUuidLike(normalizedValue) ? fallback : normalizedValue;
	}
	...
	if (Array.isArray(value)) {
		...
		return value
			.map((item) => getDisplayText(item, fallback))
			.filter((item) => item !== "—")
			.join(", ");
	}
	...
	if (typeof value === "object") {
		...
		for (const key of preferredKeys) {
			...
			if (typeof candidate === "string" && candidate.trim().length > 0) {
				return getDisplayText(candidate, fallback);
			}
		}
		...
		const scalarEntries = Object.entries(record)
			.filter(([entryKey, entryValue]) => {
				if (
					entryKey === "id" ||
					entryKey === "uuid" ||
					entryKey.endsWith("_id") ||
					entryKey.endsWith("_uuid")
				) {
					return false;
				}
			...
			.map(
				([entryKey, entryValue]) =>
					`${humanizeLabel(entryKey)}: ${getDisplayText(entryValue, fallback)}`,
			)
			.filter((entry) => !entry.endsWith(`: ${fallback}`));
...
export function getSpecimenDisplayLabel(
	specimen:
		| Pick<SpecimenPublic, "specimen_reference_id" | "id">
		| null
		| undefined,
): string {
	if (!specimen) {
	...
	const referenceId = specimen.specimen_reference_id?.trim();
	if (referenceId) {
		return referenceId;
	}

	return getDisplayText(specimen.id, "Unnamed");
}
